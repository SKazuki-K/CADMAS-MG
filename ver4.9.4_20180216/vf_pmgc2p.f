...
C     水位接続を行う(=1)か否か(=0)
      ICONWL=0                                                  ! 要修正
C
C     親を領域分割するとき、以下の数字を設定（ハードコーディング）
...
c               write(100+mgrank,*) 'c2p:',nn,ic,nrecv(ic),MGCRNK(IC)
               CALL VF_ZXMG_IRECVD(RBUF(NN),NRECV(IC),MGCRNK(IC),
     $                             IREQ,IERR)
               CALL VF_ZXMG_WAIT(IREQ,IERR)
cc               write(100+mgrank,*) rbuf(nn:nn+nrecv(ic)-1)
               NN=NN+NRECV(IC)
  610       CONTINUE
         endif
c
         CALL SET_C2P(rbuf,DBUF,MYRANK,NPROCS)
      endif
cadd20160729(e)
C
CD    -- 子の値を親が受信する --
...
c          CALL VF_ZXMG_WAIT(IREQ,IERR)
          if( ic.eq.1 ) NN=0
c          write(100+mgrank,*) dbuf(nn+1:nn+nn2)
cadd20160729(s)
...
C----------------------------------------------------------2017.03.10 strat.araki
C----------------------------------------------------------2017.03 strat.ohki
